    ...
    <form action="{{route('admin.category.update',['id'=>$category->id])}}" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" name="name" value="{{$category->name}}" class="form-control" id="exampleInputEmail1"
                   aria-describedby="emailHelp" placeholder="Enter Name Cateogory">
        </div>
        <div class="form-group">
            ...
            <textarea class="form-control" name="decription" placeholder="Entent Decription"
                      id="exampleFormControlTextarea1" rows="3">{{$category->description}}</textarea>
        </div>
        <div class="row">
            <div class="col-md-6">
                <div class="form-check">
                    <input type="checkbox" {{$category->status == '1' ? 'checked' : ''}} class="form-check-input"
            ...
            <div class="col-md-6">
                <div class="form-check">
                    <input type="checkbox" {{$category->popular == '1' ? 'checked' : ''}} class="form-check-input"
                           name="popular" id="exampleCheck1">
        ...
        <div class="form-group">
            ...
            <input type="text" name="meta_title" class="form-control" id="exampleInputEmail1"
                   aria-describedby="meta_title" value="{{$category->meta_title}}" placeholder="Enter Meta Title">
        </div>
        <div class="form-group">
            <label for="exampleInputEmail1">Meta description</label>
            <input type="text" name="meta_descrip" class="form-control" value="{{$category->meta_descrip}}"
                   id="exampleInputEmail1" aria-describedby="meta_descrip" placeholder="Enter Meta Title">
        </div>
        <div class="form-group">
            <label for="exampleInputEmail1">Meta keywords</label>
            <textarea class="form-control" name="meta_keywords" placeholder="Meta keywords"
                      id="exampleFormControlTextarea1" rows="3">{{$category->meta_keywords}}</textarea>
        </div>
        @if($category->image)
            <img src="{{asset('assets/upload/category/'.$category->image)}}" style="width: 100px">
        @endif
        <div class="form-group">
